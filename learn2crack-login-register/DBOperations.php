...
class DBOperations{
	 ...
	 private $user = 'root';
	 private $db = 'breatheasy-login-register';
	 private $pass = '';
	 private $conn;
...
public function __construct() {
 ...
 public function insertData($name,$email,$password){
 	...
 	$patientNumber = patienNo('', true);
    $hash = $this->getHash($password);
    $encrypted_password = $hash["encrypted"];
	$salt = $hash["salt"];

 	$sql = 'INSERT INTO users SET patientNumber =:patientNumber,name =:name,
    email =:email,encrypted_password =:encrypted_password,salt =:salt,created_at = NOW()';

 	$query = $this ->conn ->prepare($sql);
 	$query->execute(array('patientNumber' => $patientNumber, ':name' => $name, ':email' => $email,
 ...
 public function checkLogin($email, $password) {
    ...
    if ($this -> verifyHash($password.$salt,$db_encrypted_password) ) {
        ...
        $user["patientNumber"] = $data -> patientNumber;
        return $user;
